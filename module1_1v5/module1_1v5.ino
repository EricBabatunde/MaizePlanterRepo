...
void startBracket(SystemState newState, int startPWM, int limitPWM, String bracketName) {
  ...
  // Assign a unique filename based on the bracket and run iteration
  if (bracketName == "A") { runCountA++; currentLogFile = "/BracketA_Run" + String(runCountA) + ".csv"; }
  if (bracketName == "B") { runCountB++; currentLogFile = "/BracketB_Run" + String(runCountB) + ".csv"; }
  if (bracketName == "C") { runCountC++; currentLogFile = "/BracketC_Run" + String(runCountC) + ".csv"; }
  if (bracketName == "D") { runCountD++; currentLogFile = "/BracketD_Run" + String(runCountD) + ".csv"; } // <-- ADD THIS
  ...
  Serial.printf("\n[UI] Bracket %s Triggered. Logging to: %s\n", bracketName.c_str(), currentLogFile.c_str());
...
void setup() {
  ...
  ledcSetup(CH_L_R, 22500, 8); ledcAttachPin(PIN_L_RPWM, CH_L_R);
  ledcSetup(CH_L_L, 22500, 8); ledcAttachPin(PIN_L_LPWM, CH_L_L);
  ledcSetup(CH_R_R, 22500, 8); ledcAttachPin(PIN_R_RPWM, CH_R_R);
  ledcSetup(CH_R_L, 22500, 8); ledcAttachPin(PIN_R_LPWM, CH_R_L);
...
void loop() {
  ...
  if (currentState == MANUAL) {
    ...
    // Software E-Stop: Hard kill all movement immediately
    setMotors(1500, 1500); 
  }
  else {
    ...
    if (millis() - lastStepTime > STEP_DELAY_MS) {
      ...
      // 1. Log the stabilized speed to the Serial Monitor and LittleFS
      ...
      if (currentSweepPWM > sweepUpperLimit) {
        ...
        targetLeftRC = 1500;
        targetRightRC = 1500;
        setMotors(1500, 1500); // Stop motors at the end of the run
      } else {
    ...
    // Apply automated sweep values equally to both front tracks for straight-line travel
